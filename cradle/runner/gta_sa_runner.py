...
class PipelineRunner:
    ...
    def set_internal_params(self, *args, **kwargs):
        ...
        self.information_gathering_preprocess = InformationGatheringPreprocessProvider(gm=self.gm)
        self.information_gathering = InformationGatheringProvider(llm_provider=self.llm_provider,
                                                                  gm=self.gm,
        ...
        self.self_reflection_preprocess = SelfReflectionPreprocessProvider(gm=self.gm)
        self.self_reflection = SelfReflectionProvider(llm_provider=self.llm_provider,
                                                      gm=self.gm,
                                                      **self.provider_configs.self_reflection_provider)
        self.self_reflection_postprocess = SelfReflectionPostprocessProvider()

        self.task_inference_preprocess = TaskInferencePreprocessProvider(gm=self.gm)
        self.task_inference = TaskInferenceProvider(llm_provider=self.llm_provider,
                                               gm=self.gm,
                                               **self.provider_configs.task_inference_provider)
        self.task_inference_postprocess = TaskInferencePostprocessProvider(use_subtask=False)

        self.action_planning_preprocess = ActionPlanningPreprocessProvider(gm=self.gm)
        self.action_planning = ActionPlanningProvider(llm_provider=self.llm_provider,
                                                      gm=self.gm,
