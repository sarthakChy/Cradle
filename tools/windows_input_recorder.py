...
if platform.system() != "Windows":  # pragma: no cover - Windows-specific tool
    raise SystemExit("This GTA SA logger is Windows-only.")
...
class InputRecorder:
    ...
    def _mouse_button_name_from_message(self, message: int, mouse_data: int) -> str:
        ...
        if message in (WM_XBUTTONDOWN, WM_XBUTTONUP):
            ...
            return "x1" if button == 0x0001 else "x2"
    ...
    def _create_message_window(self) -> None:
        self._class_name = f"CradleGtaSaInputRecorderWindow_{id(self):x}"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Record GTA SA keyboard and mouse input with Windows hooks and Raw Input.")
    parser.add_argument(
        "--output",
        required=True,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Interval between state snapshots in milliseconds. Default: 10",
    )
